...
def psychrometric_calculations(dry_bulb_temperature, relative_humidity, atmospheric_station_pressure):
    ...
    humidity_ratio = np.vectorize(GetHumRatioFromRelHum)(dry_bulb_temperature, relative_humidity / 100, atmospheric_station_pressure)
    wet_bulb_temperature = np.vectorize(GetTWetBulbFromHumRatio)(dry_bulb_temperature, humidity_ratio, atmospheric_station_pressure)
    partial_vapour_pressure_moist_air = np.vectorize(GetVapPresFromHumRatio)(humidity_ratio, atmospheric_station_pressure)
    enthalpy = np.vectorize(GetMoistAirEnthalpy)(dry_bulb_temperature, humidity_ratio)
    specific_volume_moist_air = np.vectorize(GetMoistAirVolume)(dry_bulb_temperature, humidity_ratio, atmospheric_station_pressure)
    deg_saturation = np.vectorize(GetDegreeOfSaturation)(dry_bulb_temperature, humidity_ratio, atmospheric_station_pressure)
