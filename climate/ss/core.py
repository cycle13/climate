from climate.ss.compute.mean_radiant_temperature import mrt_solar_adjusted, mrt_openfield
from climate.ss.compute.comfort import utci_openfield, \
    utci_solar_adjusted, set_solar_adjusted, set_openfield

import climate

from climate.ss.compute import load_epw, to_wea, to_df, to_csv, to_pickle, load_pickle
...
class Weather(object):
    ...
    def run_sun_position(self):
        climate.ss.compute.sun.annual_sun_position(self)
        return self

    def run_psychrometrics(self):
        climate.ss.compute.psychrometrics.annual_psychrometrics(self)
        return self

    def run_sky_matrix(self):
        climate.ss.compute.sun.generate_sky_matrix(self)
        return self

    def run_pedestrian_wind(self, height=1.5, terrain="Airport runway areas"):
        climate.ss.compute.wind.pedestrian_wind_speed(self, target_wind_height=height, terrain_roughness=terrain)
        return self
