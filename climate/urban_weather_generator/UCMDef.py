...
class UCMDef(object):
    ...
    def __init__(self, bldHeight, bldDensity, verToHor, treeCoverage, sensAnthrop, latAnthrop,
        ...
        d = self.bldWidth / (sqrt(
            bldDensity))  # urban area width == sqrt(bldDensity) == ratio of bld footprint_width / urban area footprint
        ...
        self.roadConf = pow(pow(self.canAspect, 2) + 1,
                            0.5) - self.canAspect  # road-sky configuration factor (sky view factor SVF)
        self.wallConf = 0.5 * (self.canAspect + 1 -
                               pow(pow(self.canAspect, 2) + 1, 0.5)) / (
                            self.canAspect)  # wall-sky configuration factor (sky view factor SVF)
        ...
        self.facAbsor = (1 - r_glaze) * (1 - alb_wall) + r_glaze * (
                    1 - 0.75 * SHGC)  # avg facade absorptivity (-) == wall_mat_fraction * absorption + window_frac * non_solar_heat_gain
    ...
    def UCModel(self, BEM, T_ubl, forc, parameter):
        ...
        for j in range(len(BEM)):
            ...
            self.Q_ubl = self.Q_ubl + BEM[j].frac * self.bldDensity * (BEM[j].roof.sens + BEM[j].building.sensWaste * (
        ...
        self.Q_road = h_conv * (T_road - self.canTemp) * (
                    1. - self.bldDensity)  # Sensible heat from road (W/m^2 of urban area)
        ...
        for j in range(len(BEM)):
            ...
            self.Q_window = self.Q_window + BEM[j].frac * self.verToHor * R_glazing * BEM[j].wall.solRec * (
                        1. - BEM[j].building.shgc)
            self.Q_vent = self.Q_vent + BEM[j].frac * self.bldDensity * Cp_air * dens * (V_vent + V_infil) * (
                        T_indoor - T_can)
