...
class SolarCalcs(object):
    ...
    def solarcalcs(self):
        ...
        if self.dir + self.dif > 0.:
            ...
            if self.simTime.month < self.parameter.vegStart or self.simTime.month > self.parameter.vegEnd:
                ...
                alb_road = self.UCM.road.albedo * (
                            1. - self.UCM.road.vegCoverage) + self.parameter.vegAlbedo * self.UCM.road.vegCoverage
            ...
            for j in range(len(self.BEM)):
                ...
                self.BEM[j].wall.solRec = self.bldSol + (
                            1 - 2 * self.UCM.wallConf) * self.mw + self.UCM.wallConf * self.mr
            ...
            self.UCM.SolRecWall = self.bldSol + (
                        1 - 2 * self.UCM.wallConf) * self.UCM.road.albedo * self.roadSol  # Solar received by wall
    ...
    def solarangles(self):
        ...
        self.zenith = math.acos(
            math.sin(zlat) * math.sin(self.decsol) + math.cos(zlat) * math.cos(self.decsol) * math.cos(ha))
